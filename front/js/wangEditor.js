    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            if (resultText.indexOf('error|') === 0) {
				...
				//为live定制
				img_id=resultText.split('|')[0];
				resultText=resultText.split('|')[1];
				originalName=document.title;
                // 将结果插入编辑器
                img = document.createElement('img');
                img.onload = function () {
                    var html = '<img id="'+img_id+'" title="'+img_id+'" src="' + resultText + '" alt="' + originalName + '" style="max-width:100%;"/>';
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var upfile = new E.UploadFile({
            ...
            fileAccept: 'image/jpg,image/jpeg,image/png,image/gif'    // 只允许选择图片 
        });
