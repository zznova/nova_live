	...
	$.ajax({
		...
		.done(function( msg ) {
			...
			$.each(msg.list,function(k,v){
				$('#list').prepend('<div id="'+v.id+'" class="cells"><span class="date">'+v.ptime+'</span><span class="host">['+v.host+']</span><div id="c_'+v.id+'" class="content"><div id="content_'+v.id+'">'+v.content+'</div></div></div>');
			});
